...
describe Hand do
	...
	it "determines whether it doesn't have a book of cards (4 cards of the same rank)" do
		test_hand = Hand.new
		test_card1 = Card.new({:rank=>5, :suit=>"Hearts"})
		test_hand.cards << test_card1
		test_card2 = Card.new({:rank=>5, :suit=>"Clubs"})
		...
		test_card4 = Card.new({:rank=>2, :suit=>"Hearts"})
		test_hand.cards << test_card4
		expect(test_hand.book_of_rank).to eq 0
		expect(test_hand.cards.length).to eq 4
	end

	it "determines whether it has a book of cards (4 cards of the same rank)" do
		test_hand = Hand.new
		...
		test_hand.cards << test_card4
		expect(test_hand.book_of_rank).to eq 2
	...
	it "finds and creates an array of cards of requested rank, deleting them from the hand" do
		...
		test_card4 = Card.new({:rank=>2, :suit=>"Hearts"})
		test_hand.cards << test_card4
		found_cards = test_hand.find_in_hand(5)
	...
	it "returns an empty array if hand contains no cards of requested rank" do
		test_hand = Hand.new
		test_card1 = Card.new({:rank=>5, :suit=>"Hearts"})
		test_hand.cards << test_card1
		test_card2 = Card.new({:rank=>5, :suit=>"Clubs"})
		...
		test_card4 = Card.new({:rank=>2, :suit=>"Hearts"})
		test_hand.cards << test_card4
		found_cards = test_hand.find_in_hand(14)
